...
class EditAnabulWidget extends StatefulWidget {
  const EditAnabulWidget({super.key});

  @override
  State<EditAnabulWidget> createState() => _EditAnabulWidgetState();
}

class _EditAnabulWidgetState extends State<EditAnabulWidget> {
  // int currPageState = 3;
  File? petImage;
  String? petNameValue;
  String spesiesValue = "";
  ...
  // late TextEditingController _umurController, _beratController;
  ...
  void savePetData() {
    ...
    if (canSaveData) {
      print('lewat');
      Provider.of<AuthProvider>(context, listen: false)
          .eitherFailureOrUpdatePetDataSuccess(
  ...
  void initState() {
    ...
    // _umurController = TextEditingController(text: petEntity!.age.toString());
  }
  ...
  Widget build(BuildContext context) {
    ...
    petEntity = Provider.of<AuthProvider>(context, listen: false).petDetails;
    // print("token saved $accessToken");
    setState(() {
      if (petNameValue == null) {
        petNameValue = petEntity!.name ?? '';
        if (petEntity!.petCategory == 'Dog') {
          spesiesValue = 'dog';
        } else if (petEntity!.petCategory == 'Cat') {
          spesiesValue = 'cat';
        }
        if (petEntity!.petGender == 'Female') {
          jenisKelaminValue = 'male';
        } else if (petEntity!.petGender == 'Male') {
          jenisKelaminValue = 'female';
        }
        petAgeValue = petEntity!.age!;
        isSteril = petEntity!.hasBeenSterilized!;
        print(petEntity!.hasBeenSterilized!);
        isVaksin = petEntity!.hasBeenVaccinatedRoutinely!;
        isObatKutu = petEntity!.hasBeenFleaFreeRegularly!;

        petBreedName = petEntity!.petBreed!;
      }
    });
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                            petBreedName.isEmpty ? 'Pilih Ras' : petBreedName,
                            style: AppTextStyle(
                                    color: petBreedName.isEmpty
                                        ? AppColor.grey_1
                                        : AppColor.black_2)
                                .sfproBodyS(),
                          ),
